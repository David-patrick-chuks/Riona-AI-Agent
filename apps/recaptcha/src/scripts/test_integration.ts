...
(async () => {
  try {
    ...
    const correctFiles = fs
      .readdirSync(correctDir)
      .filter((f) => !f.startsWith('.'))
      .slice(0, 2);
    const incorrectFiles = fs
      .readdirSync(incorrectDir)
      .filter((f) => !f.startsWith('.'))
      .slice(0, 2);
    ...
    if (allFiles.length === 0) {
      logger.error(
        'No images found for testing. Please ensure data/correct and data/incorrect have images.'
      );
      process.exit(1);
    }
    ...
    results.forEach((res) => {
      ...
      const icon = isMatch ? '✅' : '❌';
      logger.info(
        `${icon} [${res.filename}] -> Predicted: ${res.classification} (${res.score}) | Expected: ${expected}`
      );
    });
